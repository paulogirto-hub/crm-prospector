...
<div class="space-y-6" x-data="{ 
    channel: '<?= e($template['channel']) ?>',
    vars: <?= e($template['variables'] ?: '[]') ?>,
    newVar: '',
    addVar() {
    ...
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-white">Editar Template</h1>
    </div>

    <form method="POST" action="/templates/<?= e($template['id']) ?>" class="space-y-6">
        <?= $csrfField ?>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="niche" value="<?= e($template['niche'] ?? '') ?>"
                           class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:border-purple-500 focus:outline-none">
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-xl p-6">
            ...
            <div x-show="channel === 'email'" class="mb-4">
                ...
                <input type="text" name="subject" value="<?= e($template['subject'] ?? '') ?>"
                       class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:border-purple-500 focus:outline-none">
            </div>
            ...
            <div>
                ...
                <textarea name="body" rows="10" required
                          class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:border-purple-500 focus:outline-none font-mono"><?= e($template['body']) ?></textarea>
            </div>
        </div>
